...
def predict_pergene(data, params, posterior_stats, alpha, Z_norm, group):
    num_indivs = data.G[group].shape[0]
    lambda_ = ((data.Z.T * data.maf_weights).T.matmul(data.tau)) * posterior_stats['w_g']
    beta = posterior_stats['rho'] * lambda_ + (1-posterior_stats['rho']) * lambda_ * Z_norm['mean']
    Gbeta = (data.G[group]).matmul(beta) 
    mean = torch.sigmoid(torch.matmul(data.X[group], alpha).reshape(-1,1) + Gbeta.reshape(-1,1)).view(num_indivs) 
    return mean
